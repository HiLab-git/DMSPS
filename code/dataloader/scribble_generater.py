...
############################################################################################
def scrible_2d_PerNSclices(label, interNum, iteration=[4, 10]):
    lab = label
    skeleton_map = np.zeros_like(lab, dtype=np.int32) # copy label, 全0
    for i in range(0, lab.shape[0], interNum):
        if np.sum(lab[i]) == 0:
            continue
        struct = ndimage.generate_binary_structure(2, 2)
        if np.sum(lab[i]) > 900 and iteration != 0 and iteration != [0] and iteration != None:
...
if __name__ == "__main__":
    ...
    ##############
    num = 0
    # BTCV_data_lab_path = "/mnt/data/HM/Datasets/Abdomen_BTCV/Training/label_convert"
    WORD_data_lab_path = "/mnt/data/HM/Datasets/Abdomen_word/WORD-V0.1.0-Admin_cropWL/labelsTr"
    data_lab_path = WORD_data_lab_path
    # output_path = BTCV_data_lab_path.replace('label', 'scribble')
    output_path = data_lab_path.replace('labels', 'scribblesPer'+str(interval_layer))
