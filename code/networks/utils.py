...
class UnetConv3_dropout(nn.Module):
    """
    is_batchnorm:是否使用IN进行归一化
    结果: conv3d+(IN)+ReLU+[dropout]+Conv3d+(IN)+ReLU
    """
    def __init__(self, in_size, out_size, dropout_p, is_batchnorm, kernel_size=(3,3,1), padding_size=(1,1,0), init_stride=(1,1,1)):
        super(UnetConv3_dropout, self).__init__()
...
class UnetConv3_dropout1(nn.Module):
    """
    conv3d+(IN)+ReLU+Conv3d+(IN)+ReLU
    """
    def __init__(self, in_size, out_size, dropout_p, is_batchnorm, kernel_size=(3,3,1), padding_size=(1,1,0), init_stride=(1,1,1)):
        super(UnetConv3_dropout1, self).__init__()
        ...
        if is_batchnorm:
            ...
            self.conv_conv = nn.Sequential(nn.Conv3d(in_size, out_size, kernel_size, init_stride, padding_size),
                                       nn.ReLU(inplace=True),
                                       nn.Dropout(dropout_p),
                                       nn.Conv3d(out_size, out_size, kernel_size, 1, padding_size),
                                       nn.ReLU(inplace=True))
...
class UnetUp3_CT_HM(nn.Module):
    ...
    def __init__(self, in_size, out_size, dropout_p, is_batchnorm=True):
        super(UnetUp3_CT_HM, self).__init__()
        self.conv = UnetConv3_dropout(in_size + out_size, out_size, dropout_p, 
                            is_batchnorm, kernel_size=(3,3,3), padding_size=(1,1,1)) 
        self.up = nn.Upsample(scale_factor=(2, 2, 2), mode='trilinear')

        # initialise the blocks
        for m in self.children():
            if m.__class__.__name__.find('UnetConv3_dropout') != -1: continue
            init_weights(m, init_type='kaiming')
...
class UnetUp3_CT_HM1(nn.Module):
    """upsample +'compare shape' + cat + conv(含droupout); out = conv"""
    def __init__(self, in_size, out_size, dropout_p, is_batchnorm=True):
        super(UnetUp3_CT_HM1, self).__init__()
        self.conv = UnetConv3_dropout1(in_size + out_size, out_size, dropout_p, 
                            is_batchnorm, kernel_size=(3,3,3), padding_size=(1,1,1))
        self.up = nn.Upsample(scale_factor=(2, 2, 2), mode='trilinear')

        # initialise the blocks
        for m in self.children():
            if m.__class__.__name__.find('UnetConv3_dropout1') != -1: continue
            init_weights(m, init_type='kaiming')
