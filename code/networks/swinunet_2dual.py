...
from networks.swinunet_sys_2dual import SwinTransformerSys_2dual

class SwinUNet_2dual(nn.Module):
    """
    ...
    * Reference: Hu Cao, Yueyue Wang et al:
    ...
    """    
    def __init__(self, img_size = 224, class_num=4):
        super(SwinUNet_2dual, self).__init__()
        img_size    = img_size
        if(isinstance(img_size, tuple) or isinstance(img_size, list)):
            img_size = img_size[0]
        self.num_classes = class_num
        self.swin_unet_2dual = SwinTransformerSys_2dual(img_size = img_size, num_classes=self.num_classes)
    ...
    def forward(self, x):
        ...
        if x.size()[1] == 1:
            x = x.repeat(1,3,1,1)
        logits, logits_auxi = self.swin_unet_2dual(x)
        ...
        if(len(x_shape) == 5):
            ...
            logits_auxi = torch.reshape(logits_auxi, new_shape)
            logits_auxi = torch.transpose(logits_auxi, 1, 2)
