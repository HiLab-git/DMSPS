...
# cost_matrix_dict = {'64':cost_matrix(64, device).unsqueeze(0)}
...
def neigh_penalty(input1, input2, k):
    ...
    pw_z = input1[:, :, :, :, :-1] - input2[:, :, :, :, 1:]

    pw_x = pw_x[:,:,k-1::k,:,:]
    pw_y = pw_y[:,:,:,k-1::k,:]
    pw_z = pw_z[:,:,:,:,k-1::k]

    pw_x = F.avg_pool3d(pw_x.abs().mean(1), kernel_size=(1,k,k))
    pw_y = F.avg_pool3d(pw_y.abs().mean(1), kernel_size=(k,1,k))
    pw_z = F.avg_pool3d(pw_z.abs().mean(1), kernel_size=(k,k,1))
...
def mixup_graph(input1,target_reweighted, grad1, indices, block_num=2, alpha=0.5, beta=0., gamma=0., eta=0.2, neigh_size=2, n_labels=2, 
    ...
    batch_size, _, _, _, width = input1.shape
    ...
    # unary term
    grad1_pool = F.avg_pool3d(grad1, block_size)
    unary1_torch = grad1_pool / grad1_pool.reshape(batch_size, -1).sum(1).reshape(batch_size, 1, 1,1)
    unary2_torch = unary1_torch[indices]
     
    # calculate pairwise terms
    input1_pool = F.avg_pool3d(input1 * std + mean, neigh_size)
